...
def compute_kl_feat(model, tokenizer, data_dir, save_dir, max_length, device):
    test_datasets = {}
    ...
    for file_name in os.listdir(data_dir):
        if not os.path.exists(save_dir+file_name.split('.')[0]+'.pkl'):
            ...
            for idx in tqdm(range(n_samples)):
                ...
                prompt = text
                kl = get_kl(model, tokenizer, [text], max_length, device)
                kls.append(kl)
                if len(kls) >= 300:
                    break
...
def compute_embedding(model, tokenizer, data_dir, save_dir, max_length, device):
    test_datasets = {}
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    for file_name in os.listdir(data_dir):
        if not os.path.exists(save_dir + file_name.split('.')[0] + '.pt'):
            ...
            for idx in tqdm(range(n_samples)):
                ...
                result = data[idx]['result']
                prompt = text
                embedding = get_all_embedding(model, tokenizer, [text], max_length, device)
