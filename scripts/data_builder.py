...
try:
    ...
    print("EasyDataAugmenter not found. Please install textattack package to use data augmentation.")
    EasyDataAugmenter = None
...
class DataBuilder:
    ...
    def generate_samples(self, raw_data, batch_size):
        ...
        if self.args.attack_prop > 0.0:
            eda = EasyDataAugmenter(pct_words_to_swap=self.args.attack_prop, 
        ...
        if self.args.attack_prop > 0.0:
            # apply EDA to sampled_text
            for i, one_text in enumerate(data['sampled']):
                data['sampled'][i] = eda.augment(one_text)[0]
